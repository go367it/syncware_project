...
const Devices = () => {
  ...
  return (
    <div className="text-white px-10 py-5 md:py-10 w-full">
      <table className="w-full">
        <thead>
          <tr className="divide-x divide-gray-700 text-left">
            <th scope="col" className="pl-5 pr-2">
              Device Name
            </th>
            <th scope="col" className="pl-5 pr-2">
              Variables
          ...
          {deviceData.map((j, index) => {
            return (
              <tr key={index} className="mt-5 divide-x divide-gray-700 text-gray-600 text-left">
                <td className="pl-5 pr-2 py-4">{j.name}</td>
                <td className="pl-5 pr-2 py-4">{j.variables}</td>
                <td className="pl-5 pr-2 py-4">{j.sign}</td>
                <td className="pl-5 pr-2 py-4">
                  <Switch
                    checked={j.indicator}
                    onChange={()=>onClicked(index)}
                    className={`${j.indicator ? "bg-teal-900" : "bg-gray-700"}
                      relative inline-flex flex-shrink-0 h-[34px] w-[74px] border-2 border-transparent rounded-full 
                  ...
                  >
                    ...
                    <span
                      ...
                      className={`${j.indicator ? 'translate-x-9' : 'translate-x-0'}
                        pointer-events-none inline-block h-[30px] w-[34px] rounded-full bg-white shadow-lg transform 
